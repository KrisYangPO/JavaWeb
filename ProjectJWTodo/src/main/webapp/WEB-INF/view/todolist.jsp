...
<body class="d-flex justify-content-center align-items-center vh-100 bg-light">
   <div class="container text-center">
       ...
       <!-- 新增表單 -->
       <form method="post" action="/JavaWebToDo/todolist/add" class="p-4 bg-white rounded shadow mb-4">
           <fieldset>
                   ...
                   <div>
                       <div class="form-group mb-0">
                           ...
                           <input class="form-control" type="text"
                                  ...
                                  <!-- escapeXml 可以避免有人在表單寫入 Java 程式破壞網頁 -->
                       </div>
                       <small class="text-muted">（雙擊可編輯，按 Enter 儲存）</small>
                   </div>
                   <div>
                       <input type="checkbox" ${todo.completed ? "checked" : ""}
                              onchange="location.href='/JavaWebToDo/todolist/update?id=${todo.id}&checked=' + this.checked;" />
                       已完成
                       <a href="/JavaWebToDo/todolist/delete?id=${todo.id}"
   ...
   <script type="text/javascript">
       const updateText = (event, id, input) => {
           if (event.key === 'Enter') {
               input.readOnly = true;
               const url = '/JavaWebToDo/todolist/update?id=' + id + '&text=' + encodeURIComponent(input.value);
               location.href = url;
           }
       };
   </script>
